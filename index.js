...
function findElementGreaterThan(arr, num) {
  ...
  if (Array.isArray(arr) === true) {
    ...
    if (result > num) {
      ...
      return `Число ${num} не задовільняє умову`;
    }
  }
}
...
function getSubArrayAndConvertToString(arr, startIdx) {
  ...
  if (!Array.isArray(arr)) {
    ...
    let result = arr.slice(startIdx).join(" ");
    return result;
  }
}
...
function getPositiveArrayValues(arr) {
  ...
  if (!Array.isArray(arr)) {
    return "Вхіднй параметр має бути масивом"
  } else {
    let iterator = arr.values();
    let transformToArray = Array.from(iterator);
    
    let newArray = transformToArray.filter((value) => value > 0);
    return newArray;
  }
}
...
function removeAndDouble(arr, startIdx) {
  ...
  if (!Array.isArray(arr)) {
    ...
    arr.splice(startIdx);
    let doubleArray = arr.map((value) => value * 2);
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  if(!Array.isArray(arr)) {
    ...
    let squareArray = arr.flat(1).map((element) => element ** 2);

    let filteredSquaredArray = squareArray.filter((value) => value > threshold);
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  if (!Array.isArray(arr)) {
    ...
    let sum = start.reduceRight((accumulator, currentValue) => accumulator + currentValue, 0);

    start.forEach((element) => console.log(element));
...
function expandArrayByN(arr, n) {
  ...
  if (!Array.isArray(arr) || typeof(n) !== "number") {
    ...
    let newArray = arr.flatMap((element) => Array(n).fill(element));
    return newArray;
  }
}
...
function findLongestWord(arr) {
  ...
  if (!Array.isArray(arr)) {
    ...
    let longestWord = arr.reduce((accumulator, currentValue) => currentValue.length > accumulator.length ? currentValue : accumulator);
    return longestWord;
  }
}
...
function findDuplicateElements(arr) {
  ...
  if (!Array.isArray(arr)) {
    ...
    let filtredArray = arr.filter((element, index) => arr.indexOf(element) !== index && arr.lastIndexOf(element) === index)
    return filtredArray; 
  }
}
...
function capitalizeWords(arr) {
  ...
  if (!Array.isArray(arr)) {
    ...
    let capitalizedArray = arr.map((value, index) => value[0].toUpperCase() + value.slice(1));
...
function calculateTotalPrice(arr) {
  ...
  if (!Array.isArray(arr)) {
    ...
    let totalPrice = arr.reduce((accumulator, currentValue) => accumulator + currentValue.price, 0);
    return totalPrice;
  }
}
...
function findLastNegativeNumberIndex(arr) {
  ...
  if (!Array.isArray(arr)) {
    ...
    let result = arr.findLastIndex((value) => value < 0);
    return result;
  }
}
